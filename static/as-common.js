...
function build_ocoins_str(p) {
    var ocoins_info = {
        ...
        "rft.jtitle":
            ...
            p.pid +
            " [" +
            p.category.substring(0, p.category.indexOf(".")) +
            "]",
...
function addPapers(num, dynamic) {
    ...
    for (var i = 0; i < num; i++) {
        ...
        if (ix >= papers.length) {
            if (!showed_end_msg) {
                if (ix >= numresults) {
                    ...
                    var msg =
                        "You hit the limit of number of papers to show in one result.";
                }
                root.append("div")
                    .classed("msg", true)
                    .html(msg);
        ...
        var discuss_text =
            p.num_discussion === 0
                ? "discuss"
                : "discuss [" + p.num_discussion + "]";
        var discuss_color = p.num_discussion === 0 ? "black" : "red";
        var review_span = ldiv
            .append("span")
            .classed("sim", true)
            .attr(
                "style",
                "margin-left:5px; padding-left: 5px; border-left: 1px solid black;"
            )
            .append("a")
        ...
        var lib_state_img =
            p.in_library === 1 ? "static/saved.png" : "static/save.png";
        var saveimg = ldiv
            .append("img")
            ...
            (function(pid, elt) {
                return function() {
                    if (username !== "") {
                        ...
                        $.post("/libtoggle", { pid: pid }).done(function(data) {
                        ...
                        alert(
                            "you must be logged in to save papers to library."
                        );
                    }
                };
            })(p.pid, saveimg)
        ...
        if (ix < tweets.length) {
            ...
            for (var j = 0, m = ix_tweets.length; j < m; j++) {
                ...
                var act_fun = (function(elt, txt, tname, tid, imgelt) {
                    // mouseover handler: show tweet text.
                    return function() {
                        ...
                        elt.append("div")
                            ...
                            .attr(
                                "style",
                                "font-weight:bold; color:#05f; text-decoration:none;"
                            )
